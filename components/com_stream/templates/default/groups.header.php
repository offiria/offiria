...
<ul class="nav nav-pills filter">
	...
	if (count($categories) > 0) {
		...
		<?php if (JRequest::getVar('filter') == 'category' && JRequest::getVar('category_id') != NULL) {
		...
		<ul class="dropdown-menu pull-right">
			<li><a href="<?php echo JRoute::_('index.php?' . http_build_query( array_merge($get, array('filter' => 'category', 'category_id'=> '' )) ) ); ?>">
			<?php echo JText::_('COM_STREAM_LABEL_DROPDOWN_ALL'); ?></a></li>		
			<li class="divider"></li>  
			<?php 
			foreach ($categories as $category) { ?>
				<li><a href="<?php echo JRoute::_('index.php?' . http_build_query( array_merge($get, array('filter' => 'category', 'category_id'=>$category->id )) ) ); ?>">&diams;&nbsp;<?php echo $category->category; ?></a></li>
			<?php }	?>
		</ul>
		</li>
	<?php } ?>
